...
motor_lift = Motor(PORT_LIFT, Direction.COUNTERCLOCKWISE)
...
try:
    while True:
        ...
        steering, _ = remote.joystick_right()
        ...
        if Button.A in buttons:
            motor_lift.dc(-ARM_DC)  # down
        elif Button.B in buttons:
            motor_lift.dc(ARM_DC)  # up
        else:
            motor_lift.dc(0)

        _, left_ud = remote.joystick_left()
        if left_ud < 0:
